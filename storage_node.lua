...
local function after_place_node(pos, placer)
    ...
    simple_amm.set_owner(meta, player_name)
    simple_amm.set_infotext(meta, ("External storage by: %s"):format(player_name))
end
...
local function on_construct(pos)
    ...
    simple_amm.set_mesein(meta, 0)
    simple_amm.set_title(meta, "storage@" .. minetest.pos_to_string(pos))
end
...
local function allow_metadata_inventory_put(pos, listname, index, stack, player)
    if stack:get_wear() ~= 0 or not simple_amm.util.can_access(player, pos) then
        return 0
